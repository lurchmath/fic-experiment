...
class Statement extends LC {
  ...
  toString (options) {
    ...
    ////////////////////
    debug(`Converting this to a string:`)
    this.inspect()
    ...
    if ( options && options.ID && options.ID!=='free' ||
         options && options.ID && options.ID==='free' && this.isAFreeConstant()
       ) {
  ...
  isAFreeConstant () {
    let dec=this.getAttribute('declared by')
  ...
  //   (a) it is inside a Declaration that declares it or declared by a Let
  ...
  isBound () {
    let dec=this.getAttribute('declared by')
    if (dec instanceof Statement ||
        dec.isAnActualDeclaration() &&
        (dec.isAncestorOf(this) || (dec.declaration === 'variable'))
      ) { return true }
    return false
  }
  // candy
