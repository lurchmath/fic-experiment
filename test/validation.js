...
let validate = ( problem , opts ) => {
   ...
   let options = (!opts) ? { Conc: true , Scopes: true , Bound: true } : opts
...
suite( 'FIC Validation', () => {
  ...
  test( 'Simple correct things validate correctly.', () => {
     expect(validate('{ :A A }')).to.be('{ :A A ✔︎ }')
     expect(validate('{ :{ :A B } :A B }')).to.be('{ :{ :A B } :A B ✔︎ }')
     expect(validate('{ :{ :A B } :{ :B C } { :A C } }')).to.be('{ :{ :A B } :{ :B C } { :A C ✔︎ } }')
     expect(validate('{ :{ :A B } { :{ :B C } :A C } }')).to.be('{ :{ :A B } { :{ :B C } :A C ✔︎ } }')
  } )
  ...
  test( 'Simple incorrect or mixed things validate correctly.', () => {
     ...
     expect(validate('{ A A }')).to.be('{ A ✗ A ✔︎ }')
     expect(validate('{ A { :B A } }')).to.be('{ A ✗ { :B A ✔︎ } }')
     expect(validate('{ A { A B } B }')).to.be('{ A ✗ { A ✔︎ B ✗ } B ✔︎ }')
     expect(validate('{ A { :A B } B }')).to.be('{ A ✗ { :A B ✗ } B ✔︎ }')
  ...
  test( 'And a few mimicking rules of inference for kicks.', function () {
    ...
    let proof = '{ :{ :{ A B } and(A,B) }'
              ...
              + '   A                    '
              + '}'
    let validproof = '{ :{ :{ A B } and(A,B) } :{ :and(A,B) A B } :B :C :A B ✔︎ and(A,B) ✔︎ A ✔︎ }'
    ...
    proof = '{               '
          ...
          + '  R             '
          + '}               '
    validproof = '{ :{ :or(P,Q) :{ :P R } :{ :Q R } R } :{ :Q W R } :{ :P S R U } :or(W,U) :or(P,Q) R ✔︎ }'
    expect(validate(proof)).to.be(validproof)
    proof = '{               '
          + '  :{ :or(P,Q)   '
          + '     :{ :P R }  '
  ...
  test( 'Testing valdation involving declarations.', () => {
    ...
    expect(validate('{ :Declare{ x { :P W(x) Z(x) } } :P Declare{ x { :P W(x) } } W(x) }'))
            .to.be('{ :Declare{ x✓ { :P W(x) Z(x) } } :P Declare{ x✗ { :P W(x) } } ✔︎ W(x) ✔︎ }')
    ...
    let divalg= '{                       '
      ...
      expect(validate('{ :{ :{ :Let{ x W(x) } Z(x) } ~All(x,Implies(W(x),Z(x))) } { :Let{ x W(x) } Z(x) } ~All(x,Implies(W(x),Z(x))) }'))
              .to.be('{ :{ :{ :Let{ x✓ W(x) } Z(x) } ~All(x✓,Implies(W(x),Z(x))) } { :Let{ x✓ W(x) } Z(x) ✗ } ~All(x✓,Implies(W(x),Z(x))) ✔︎ }')
  } )
