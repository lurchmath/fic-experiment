...
suite( 'Basics', () => {
  ...
  test( 'Identify actual Identifiers, actual Statements, and actual Declarations correctly.', () => {
    ...
    expect(lc('P(x,y)').first.isAnActualStatement() ).to.be( false )
    expect(lc('P(x,y)').first.isAnActualIdentifier() ).to.be( true )
    expect(lc('P(x,y)').first.isAnActualDeclaration() ).to.be( false )
    expect(lc('{ P(x,y) }').first.isAnActualStatement() ).to.be( true )
    expect(lc('{ P(x,y) }').first.isAnActualIdentifier() ).to.be( false )
    expect(lc('{ P(x,y) }').first.isAnActualDeclaration() ).to.be( false )
    ...
    expect( D.first.isAnActualStatement() ).to.be( true )
    expect( D.first.isAnActualIdentifier() ).to.be( true )
    expect( D.first.isAnActualDeclaration() ).to.be( false )
    expect( L.first.isAnActualStatement() ).to.be( true )
    expect( L.first.isAnActualIdentifier() ).to.be( true )
    expect( L.first.isAnActualDeclaration() ).to.be( false )
    expect( D.child( 1 ).isAnActualStatement() ).to.be( true )
    expect( D.child( 1 ).isAnActualIdentifier() ).to.be( false )
    expect( D.child( 1 ).isAnActualDeclaration() ).to.be( false )
    expect( L.child( 1 ).isAnActualStatement() ).to.be( true )
    expect( L.child( 1 ).isAnActualIdentifier() ).to.be( false )
    expect( L.child( 1 ).isAnActualDeclaration() ).to.be( false )
  } )
